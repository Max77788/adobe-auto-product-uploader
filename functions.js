...
async function postWithRetry(url, headers, newOption, maxRetries = 3) {
  ...
  while (attempt < maxRetries) {
    attempt++;
    try {
      console.log(`[DEBUG] Attempt ${attempt}: Sending POST request to Magento API... URL: ${url}`);
...
async function createSimpleProduct(config_product_id, config_product_name, attribute_set_id, quantity, quantity_price, custom_attributes) {
    ...
    const result = await postWithRetry(`${process.env.PRINTPRONTO_API_BASE_URL}/products`, {
        "Authorization": `Bearer ${process.env.PRINTPRONTO_API_TOKEN}`,
        "Content-Type": "application/json"
    }, product, 3);
...
async function assignSimpleProductToConfigurable(child_product_sku, config_product_sku) {
    ...
    const url = `${process.env.PRINTPRONTO_API_BASE_URL}/configurable-products/${config_product_sku}/child`;
...
async function addImageToProduct(imageUrl, productName, positionNumber, config_product_sku, isThumbnail = false) {
    ...
    try {
        ...
        const response_post_image = await fetch(`${process.env.PRINTPRONTO_API_BASE_URL}/products/${config_product_sku}/media`, {
            method: 'POST',
            headers: {
                "Authorization": `Bearer ${process.env.PRINTPRONTO_API_TOKEN}`,
                "Content-Type": "application/json"
...
/** Try to read a variant's size name from common shapes your ESP may use */
function getVariantSizeName(variant) {
  // Adjust these paths to match the actual variant payload
  // Try the most likely shapes first:
  if (variant.Attributes?.Sizes?.Values[0]?.Name) return String(variant.Attributes.Sizes.Values[0]?.Name).trim();
  if (variant.Attributes?.Size?.Values[0]?.Name)
    return String(variant.Attributes.Size.Values[0]?.Name).trim();
  /*
    // Some payloads store it as a direct field:
  if (variant.Size?.Name) return String(variant.Size.Name).trim();
  if (variant.Size) return String(variant.Size).trim();
  */

  // Fallback: sometimes variant name includes size (e.g., "Sticker - 3x5 in")
  // You can improve this parser later.
  return null;
}
...
async function processProduct(req) {
    try {
      ...
      console.log(
        "[DEBUG] Fetching product with esp_product_id:",
        esp_product_id
      );
      const product = await fetchProduct(esp_product_id);
      console.log("[DEBUG] Fetched product:", product);

      // Build a size-indexed price table from variants
      const variantPriceIndex = new Map(); // sizeLabel -> [{quantity, price}]
      if (Array.isArray(product.Variants) && product.Variants.length) {
        for (const v of product.Variants) {
          const sizeName = getVariantSizeName(v);
          if (!sizeName) continue;

          const entries = variantPriceIndex.get(sizeName) || [];
          for (const p of v.Prices || []) {
            if (!p?.Quantity?.From || typeof p?.Cost !== "number") continue;
            entries.push({
              quantity: p.Quantity.From,
              price: round2(p.Cost * price_multiplier),
            });
          }
          variantPriceIndex.set(sizeName, entries);
        }

        // Optional: dedupe by quantity, keeping the lowest price for that tier
        for (const [k, arr] of variantPriceIndex) {
          const byQty = new Map();
          for (const row of arr) {
            const q = String(row.quantity);
            byQty.set(
              q,
              byQty.has(q) ? Math.min(byQty.get(q), row.price) : row.price
            );
          }
          variantPriceIndex.set(
            k,
            [...byQty.entries()].map(([quantity, price]) => ({
              quantity,
              price,
            }))
          );
      ...
      const globalQuantitiesPrice = [];
      console.log(
        "[DEBUG] Initialized attributeMapping and globalQuantitiesPrice."
      );
      ...
      if (
        ...
        attributeMapping.Colors = {
          ...
          productValues:
            (product.Attributes.Colors && product.Attributes.Colors.Values) ||
            [],
        };
      } else {
        console.log(
          "[DEBUG] Colors attribute not provided or missing in product."
        );
      }
      ...
      if (size_attribute_id && product.Attributes && product.Attributes.Sizes) {
        ...
        attributeMapping.Sizes = {
          ...
          productValues:
            (product.Attributes.Sizes && product.Attributes.Sizes.Values) || [],
        };
      } else {
        console.log(
          "[DEBUG] Sizes attribute not provided or missing in product."
        );
      }
      ...
      if (
        ...
        console.log(
          "[DEBUG] Materials attribute not provided or missing in product."
        );
      }
      ...
      if (
        ...
        console.log(
          "[DEBUG] Shapes attribute not provided or missing in product."
        );
      }
      ...
      if (quantity_attribute_id && product.Prices) {
        console.log(
          "[DEBUG] Found product.Prices; processing quantity prices."
        );
        for (const obj of product.Prices) {
          ...
          globalQuantitiesPrice.push(quantityObject);
        }
      } else {
        console.log(
          "[DEBUG] No direct product.Prices found; processing variants for quantity prices."
        );
        product.Variants.forEach((variant) => {
          variant.Prices.forEach((obj) => {
            ...
            console.log(
              "[DEBUG] Adding variant quantity object:",
              quantityObject
            );
              globalQuantitiesPrice.push(quantityObject);
              const byQty = new Map();
              for (const row of globalQuantitiesPrice) {
                const q = String(row.quantity);
                byQty.set(
                  q,
                  byQty.has(q) ? Math.min(byQty.get(q), row.price) : row.price
              ...
              globalQuantitiesPrice.length = 0;
              for (const [quantity, price] of byQty.entries()) {
                globalQuantitiesPrice.push({ quantity, price });
              }
          });
        });
      }
      ...
      const finalResult = {};
      console.log(
        "[DEBUG] Starting to iterate over attributeMapping keys:",
        Object.keys(attributeMapping)
      );

      for (const [attrKey, data] of Object.entries(attributeMapping)) {
        console.log(
          `[DEBUG] Processing attribute group: ${attrKey} (ID: ${data.attribute_id})`
        );
        ...
        console.log(
          `[DEBUG] Fetching existing options for attribute ${attrKey} with ID: ${data.attribute_id}`
        );
        const existingOptions = await findAttributeOptions(data.attribute_id);
        console.log(
          `[DEBUG] Existing options for ${attrKey}:`,
          existingOptions
        );
        const additions = [];
        ...
        for (const option of data.productValues) {
          ...
          console.log(
            `[DEBUG] Checking option "${optionName}" for attribute ${attrKey}`
          );
          const optionExists = existingOptions.some(
            (opt) => opt.label === optionName
          );
          console.log(
            `[DEBUG] Does option "${optionName}" exist?`,
            optionExists
          );
          if (!optionExists) {
            console.log(
              `[DEBUG] Option "${optionName}" not found. Adding new option.`
            );
            const addResult = await addNewSingleOptionToTheAttribute(
              optionName,
              data.attribute_id
            );
        ...
        console.log(
          `[DEBUG] Completed processing for attribute group: ${attrKey}`
        );
      }

      // Process the Quantities attribute separately
      console.log(
        "[DEBUG] Fetching existing quantity options for quantity attribute with ID:",
        quantity_attribute_id
      );
      const existingQuantityOptions = await findAttributeOptions(
        quantity_attribute_id
      );
      console.log(
        "[DEBUG] Existing quantity options:",
        existingQuantityOptions
      );

      console.log(
        "[DEBUG] Processing globalQuantitiesPrice for quantity options."
      );
      for (const quantity of globalQuantitiesPrice) {
        console.log(
          `[DEBUG] Processing quantity: ${quantity.quantity}, Price: ${quantity.price}`
        );
        const quantityExists = existingQuantityOptions.some(
          (q) => q.label.toString() === quantity.quantity.toString()
        );
        console.log(
          `[DEBUG] Does quantity ${quantity.quantity} exist?`,
          quantityExists
        );
        if (!quantityExists) {
          console.log(
            `[DEBUG] Quantity ${quantity.quantity} not found. Adding it.`
          );
          const addResult = await addNewSingleOptionToTheAttribute(
            quantity.quantity,
            quantity_attribute_id
          ...
          console.log(
            `[DEBUG] Added quantity option for ${quantity.quantity}:`,
            addResult
          );
      ...
      const preparedColorOptions = await findAttributeOptions(
        color_attribute_id
      );
      const preparedMaterialOptions = await findAttributeOptions(
        material_attribute_id
      );
      const preparedShapeOptions = await findAttributeOptions(
        shape_attribute_id
      );
      const preparedQuantityOptions = await findAttributeOptions(
        quantity_attribute_id
      );
      ...
      console.log(
        "[DEBUG] Setting quantity options for product configuration."
      );
      for (const obj of globalQuantitiesPrice) {
        const option_value = preparedQuantityOptions.find(
          (opt) => opt.label.toString() === obj.quantity.toString()
        );
        console.log(
          `[DEBUG] Setting quantity option: ${
            option_value ? option_value.label : "not found"
          } for quantity ${obj.quantity}`
        );
        ...
        if (!option_value) {
            console.warn(`[DEBUG] Quantity option "${obj.quantity}" not found; skipping push.`);
            continue;
        }
      ...
      if (product?.Attributes?.Colors?.Values) {
        ...
        for (const obj of product.Attributes.Colors.Values) {
          try {
            const option_value = preparedColorOptions.find(
              (opt) => opt.label.toString() === obj.Name.toString()
            );
            console.log(
              `[DEBUG] Setting color option: ${
                option_value ? option_value.label : "not found"
              } for color ${obj.Name}`
            );
            if (!option_value) {
              console.error(
                `[DEBUG] Option for color "${obj.Name}" was not found in preparedColorOptions. Skipping this attribute option.`
              );
              continue;
            }
            await setOptionOfConfigProductAttribute(
            ...
            console.error(
              `[DEBUG] Error setting color option for ${obj.Name}:`,
              error
            );
            continue;
          }
      ...
      if (product?.Attributes?.Sizes?.Values) {
        ...
        for (const obj of product.Attributes.Sizes.Values) {
          ...
          const option_value = preparedSizeOptions.find(
            (opt) => opt.label.toString() === cleanedName
          );
          console.log(
            `[DEBUG] Setting size option: ${
              option_value ? option_value.label : "not found"
            } for size ${cleanedName}`
          );
          ...
          if (!option_value) {
            console.error(
              `[DEBUG] Option for "${cleanedName}" was not found in preparedSizeOptions. Skipping this attribute option.`
            );
            continue;
          }
      ...
      if (product?.Attributes?.Materials?.Values) {
        console.log(
          "[DEBUG] Setting material options for product configuration."
        );
        for (const obj of product.Attributes.Materials.Values) {
          const option_value = preparedMaterialOptions.find(
            (opt) => opt.label.toString() === obj.Name.toString()
          );
          console.log(
            `[DEBUG] Setting material option: ${
              option_value ? option_value.label : "not found"
            } for material ${obj.Name}`
          );
          if (!option_value) {
            console.error(
              `[DEBUG] Option for material "${obj.Name}" was not found in preparedMaterialOptions. Skipping this attribute option.`
            );
            continue;
          }
          await setOptionOfConfigProductAttribute(
      ...
      if (product?.Attributes?.Shapes?.Values) {
        ...
        for (const obj of product.Attributes.Shapes.Values) {
          const option_value = preparedShapeOptions.find(
            (opt) => opt.label.toString() === obj.Name.toString()
          );
          console.log(
            `[DEBUG] Setting shape option: ${
              option_value ? option_value.label : "not found"
            } for shape ${obj.Name}`
          );
          if (!option_value) {
            console.error(
              `[DEBUG] Option for shape "${obj.Name}" was not found in preparedShapeOptions. Skipping this attribute option.`
            );
            continue;
          }
          await setOptionOfConfigProductAttribute(
      ...
      const safeQuantityOptions = usedQuantityOptions.length
        ? usedQuantityOptions
        : [{ label: "", value: "" }];
      const safeSizeOptions = usedSizeOptions.length
        ? usedSizeOptions
        : [{ label: "", value: "" }];
      const safeShapeOptions = usedShapeOptions.length
        ? usedShapeOptions
        : [{ label: "", value: "" }];
      const safeColorOptions = usedColorOptions.length
        ? usedColorOptions
        : [{ label: "", value: "" }];
      const safeMaterialOptions = usedMaterialOptions.length
        ? usedMaterialOptions
        : [{ label: "", value: "" }];

      // Create configurable product combinations via nested loops
      console.log(
        "[DEBUG] Starting to create simple products for every combination of options (size-specific pricing)."
      );

      // For each size option, pick the correct (size-scoped) quantity/price tiers
      for (const sizeOption of usedSizeOptions.length
        ? usedSizeOptions
        : [{ label: "", value: "" }]) {
        const sizeLabel = replaceFractionsWithDecimals(
          String(sizeOption.label || "").trim()
        );
        const sizeQtyPriceList =
          (sizeLabel && variantPriceIndex.get(sizeLabel)) ||
          globalQuantitiesPrice;


        if (!sizeQtyPriceList || !sizeQtyPriceList.length) {
          console.warn(
            `[DEBUG] No quantity/price tiers found for size "${sizeLabel}". Skipping size.`
          );
          continue;
        }

        // For each quantity that exists for THIS SIZE, find the matching quantity option id
        for (const { quantity, price } of sizeQtyPriceList) {
          const qtyOption = preparedQuantityOptions.find(
            (opt) => String(opt.label) === String(quantity)
          );
          if (!qtyOption) {
            console.warn(
              `[DEBUG] Quantity option "${quantity}" not found in preparedQuantityOptions. Skipping.`
            );
            continue;
          }

          // Iterate other dims (shapes/colors/materials) as you had them
          for (const shapeOption of usedShapeOptions.length
            ? usedShapeOptions
            : [{ label: "", value: "" }]) {
            for (const colorOption of usedColorOptions.length
              ? usedColorOptions
              : [{ label: "", value: "" }]) {
              for (const materialOption of usedMaterialOptions.length
                ? usedMaterialOptions
                : [{ label: "", value: "" }]) {
                const combinationName = `${product.Name} Q:${quantity} S:${sizeOption.label} Sh:${shapeOption.label} C:${colorOption.label} M:${materialOption.label}`;

                const custom_attributes = [
                  {
                    attribute_code: size_attribute_label,
                    value: sizeOption.value,
                  },
                  {
                    attribute_code: color_attribute_label,
                    value: colorOption.value,
                  },
                  {
                    attribute_code: shape_attribute_label,
                    value: shapeOption.value,
                  },
                  {
                    attribute_code: material_attribute_label,
                    value: materialOption.value,
                  },
                  {
                    attribute_code: quantity_attribute_label,
                    value: qtyOption.value,
                  },
                ];
                ...
                const simpleProductSku = await createSimpleProduct(
                  ...
                  attribute_set_id,
                  quantity, // qty
                  price*quantity, // <-- size-specific price for this quantity
                  custom_attributes
                );

                await assignSimpleProductToConfigurable(
                  simpleProductSku,
                  printpronto_config_product_sku
                );
                console.log(
                  `[DEBUG] Created & assigned: ${simpleProductSku} (${combinationName}) @ ${price}`
                );
              }
            }
          }
